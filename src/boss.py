...
class Boss:
    ...
    def __init__(self, x, y, dungeon_runs=0):
        ...
        sheet_h = sprite.get_height()
        scaled = pygame.transform.scale(
            sprite,
            (sheet_w * 2, sheet_h * 2)
        )
        self.sprite_sheet = scaled
    ...
    def update(self, player_rect, walls, collision_map, tile_size):
        ...
        if dist > 0:
            ...
            if self.nav_mode == "path" and now < self.nav_mode_until:
                ...
                if self.path and self.path_index < len(self.path):
                    ...
                    target_x, target_y = tile_to_world_center(
                        next_tile[0], next_tile[1], tile_size
                    )
            ...
            nav_dist = (to_x * to_x + to_y * to_y) ** 0.5
            if nav_dist > 0:
            ...
            if any(test_rect.colliderect(w) for w in walls):
                ...
                test_x.x += dx
                if any(test_x.colliderect(w) for w in walls):
                    dx = 0.0

                test_y = self.rect.copy()
                test_y.y += dy
                if any(test_y.colliderect(w) for w in walls):
                    dy = 0.0
            else:
